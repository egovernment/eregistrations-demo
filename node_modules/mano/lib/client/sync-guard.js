...
send = function () {
	var data = keys(localStorage).filter(function (key) {
		...
		if (value == null) {
			// Can be result or very rare race condition (it actually happened)
			// For more info see warning in this section:
			// https://html.spec.whatwg.org/multipage/webstorage.html#localStorageEvent
			return;
		}
...
server.on('sync', function (serverHash) {
	...
	console.trace("Server hash:", serverHash);
