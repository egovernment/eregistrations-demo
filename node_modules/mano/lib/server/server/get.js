...
module.exports = function (appRouters) {
	return function (req, res, next) {
		...
		deferred(result.result).done(function (data) {
			...
			if (result.conf.statusCode) res.statusCode = result.conf.statusCode;
			if (isStream(data)) {
				data.pipe(res);
				return;
			}
			if (Buffer.isBuffer(data)) isCustomType = true;
			if (!isCustomType && isObject(data)) return sendJson.call(res, data);
			if (data != null) res.end(String(data));
			else res.end();
		}, sendError.bind(res));
	};
};
